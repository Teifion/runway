"""
Exception ignored in: <socket object at 0x7ffbf519e9a8>
ResourceWarning: unclosed <socket object at 0x7ffbf519e9a8>
"""
...
from venustate.core.cli.lib import cli_f
import transaction
import codecs
...
def replicate(options):
    ...
    # Pull the file
    # os.system("scp -r root@venustate.com:/var/www/wsgi/latest_backup.sql ../latest_backup.sql;")
    if not options.quick:
        os.system("scp -r root@venustate.com:/tmp/temp_duplicate.sql ../latest_backup.sql;")
...
def dev_app(options):
    ...
    # import venustate.models
    # import venustate.views
    
    # os.system("cd {}; ../bin/initialize_venustate_db development.ini; ../bin/pserve development.ini --reload;".format(config['dev_path']))
...
def replicate_for_test():
    ...
    # export PGPASSWORD="ImmortalZerglingMarines1"; dropdb --host=localhost --username=dash dash_mock
    ...
    # export PGPASSWORD="ImmortalZerglingMarines1"; createdb --host=localhost --username=dash dash_mock
    
    print("Installing SQL")
    # pg_restore --host=localhost --user=postgres --dbname=dash_mock latest.sql
    os.system("{pgbin}/psql {conn} -d {db_name} -f {path}".format(
        pgbin   = config["pgbin"],
        conn    = connect_string,
    ...
    # bootstrap()
    # from venustate.models import (
    #     DBSession,
    # )
    # DBSession.execute("""""")
    # DBSession.commit()
    
    # return dev_app(options)
    # if run_ini_script:
    #     os.system("cd {}; ../bin/initialize_venustate_db development.ini;".format(config['dev_path']))
...
def run_tests(options):
    ...
    if options.coverage:
        os.system("cd {}/venustate; ../../venv/bin/nosetests --cover-package=venustate --cover-erase --with-coverage --cover-html --cover-html-dir=../../coverage --cover-xml --cover-xml-file=../../coverage.xml".format(config['dev_path']))
        ...
        # -qf
        if options.quick:
            suite = suite.discover("%s/venustate/core/testing/" % config['dev_path'], pattern="one_test_file.py")
            unittest.TextTestRunner(verbosity=verbosity).run(suite)
            
        else:
            verbosity += 1
            suite = suite.discover("%s/venustate/" % config['dev_path'], pattern="*_tests.py", top_level_dir=config['dev_path'])
            unittest.TextTestRunner(verbosity=verbosity).run(suite)
            
            # os.system("cd {}/venustate; ../../venv/bin/nosetests --processes=12 --verbosity={}".format(config['dev_path'], verbosity))
...
def cron(options):
    ...
    from venustate.core.cron.lib import cron_f
...
def uninstall(options):
    ...
    from venustate.core import plugins
    from venustate.core.plugins.lib import find
    
    for plugin_name in find.scan_for_plugins(config['folder_path']):
        if module_name == plugin_name:
            exec("from venustate.plugins import %s" % plugin_name, plugins.__dict__)
...
def quick_test(options):
    ...
    from venustate.core.system.jobs.prune_logs import PruneLogsJob
    j = PruneLogsJob()
    j.perform_job(None)
...
def main():
    ...
    # from venustate.core.cli import commands
    from venustate.core.commands import execute_command
