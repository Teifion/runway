...
def get_profile(user_id):
    the_profile = DBSession.query(WordyProfile).filter(WordyProfile.user == user_id).first()
...
def get_game_list(user_id, mode):
    ...
    return DBSession.query(WordyGame.id, User.username, WordyGame.turn, WordyGame.players).filter(*filters)

def find_user(identifier):
    if type(identifier) == str:
        found = DBSession.query(User.id).filter(User.username == identifier).first()
        if found == None:
            return None
        return config['get_user']({'id':found[0], 'name':identifier})
    
    elif type(identifier) == int:
        found = DBSession.query(User.username).filter(User.id == identifier).first()
        if found == None:
            return None
...
def new_game(players, rematch=None):
    ...
    # Get game ID
    game_id = DBSession.query(WordyGame.id).filter(
...
def get_moves(game_id):
    return DBSession.query(WordyMove).filter(WordyMove.game == game_id).order_by(WordyMove.timestamp.asc())
...
def get_names(players):
    ...
    names = {}
    for i, n in DBSession.query(config['User'].id, config['User'].username).filter(config['User'].id.in_(players)):
        names[i] = n
    return names
...
def end_game(the_game):
    ...
    profiles = DBSession.query(WordyProfile).filter(WordyProfile.user.in_(the_game.players))
...
def completed_games(user_id, opponent_id=None):
    ...
    return DBSession.query(func.count(WordyGame.id)).filter(*filters).first()[0]
...
def check_for_install():
    r = DBSession.query(WordyWord).first()
    if r is None:
        return False
    return True
...
def find_match(profile):
    ...
    for game in DBSession.query(WordyGame.players).filter(*filters):
    ...
    opponent = DBSession.query(WordyProfile.user).filter(
...
def swap_letters(the_game, player_id):
    ...
    moves = tuple(DBSession.query(WordyMove.swap).filter(WordyMove.game == the_game.id).order_by(WordyMove.id.desc()).limit(len(the_game.players)+1))
